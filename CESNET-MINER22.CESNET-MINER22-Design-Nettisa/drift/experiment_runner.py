...
df = pd.read_csv("../../datasets/CESNET-MINERS22-Nettisa/miners_design.csv")
...
if DATE:
    ...
    df = df.sample(frac=1).reset_index(drop=True)
    df = df.reset_index()
    feat_names = df.columns.drop("LABEL")
...
with open('../../datasets/CESNET-MINERS22-Nettisa/drift/logs_cesnet_miners22_nettisa-design-retrained.pkl', 'wb') as outp:
