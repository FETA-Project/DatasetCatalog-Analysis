...
def load_csv_dataset(path: str):
    ...
    with open('miners-design.csv',mode="w") as file:
        file.write(str(",".join(header))+"\n")

    formatted_df.to_csv("miners-design.csv", sep=',', index=False, encoding='utf-8',quoting=csv.QUOTE_NONE, escapechar=' ', header=False, mode = "a")

load_csv_dataset("decrypto_dataset_design.csv")
